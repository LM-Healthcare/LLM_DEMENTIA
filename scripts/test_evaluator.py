...
def step(code: str | None, confidence: float = 0.8) -> dict:
    if code is None:
        return {
            "feasible": True,
            "patient_code": "T",
...
def main() -> None:
    ...
    evaluation = evaluate_batch(pipelines, ["AD", "VAD"])
    step1 = evaluation["metrics_per_step"]["step1"]
    records = evaluation["records"]

    checks = {
        "n_total include output invalido": step1["n_total"] == 2,
        "n_invalid": step1["n_invalid"] == 1,
        "accuracy intention-to-evaluate": step1["accuracy"] == 0.5,
